...
public interface ProvinciaRepositorio extends JpaRepository<Provincia, Integer> {
    ...
    @Query("SELECT new com.safa.enviovital.dto.ProvinciaDTO(p.nombre) FROM Provincia p")
    List<ProvinciaDTO> findAllProvinciasDTO();

    @Query("SELECT new com.safa.enviovital.dto.ProvinciaDTO(p.nombre) FROM Provincia p WHERE p.id = :id")
